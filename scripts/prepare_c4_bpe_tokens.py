...
DEFAULT_TRAIN_TOKENS = 1_310_720_000
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--max-train-tokens", type=int, default=DEFAULT_TRAIN_TOKENS)
    parser.add_argument("--max-validation-tokens", type=int, default=DEFAULT_VALIDATION_TOKENS)
...
def encode_c4_split(
    *,
    ...
    """Encode one C4 split into a contiguous int32 token file.
    ...
    Args:
        ...
        max_tokens: Maximum number of tokens to write.
    ...
    Raises:
        ...
        FileExistsError: If output or temporary output exists without overwrite.
    """
    ...
    if output_path.exists() and not overwrite:
        print(f"Token file already exists, skipping: {output_path}")
        return _read_existing_metadata(output_path)
    ...
    with tmp_path.open("wb") as destination:
        ...
        try:
            for text in iter_c4_texts(shards):
...
def _read_existing_metadata(output_path: Path) -> dict[str, Any]:
    """Read existing metadata for a skipped token file.

    Args:
        output_path: Token file whose sidecar metadata should be loaded.

    Returns:
        Metadata dictionary, or a minimal dictionary when the sidecar is absent.
    """
