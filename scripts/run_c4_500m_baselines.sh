...
wandb_group="${WANDB_GROUP:-c4-500m}"
...
if [[ "${RUN_ALF:-1}" == "1" ]]; then
  "${train_cmd[@]}" "${torchrun_args[@]}" experiments/qwen3_moe_c4_500m_alf.py "${common_overrides[@]}"
fi
...
if [[ "${RUN_AUX:-1}" == "1" ]]; then
  "${train_cmd[@]}" "${torchrun_args[@]}" experiments/qwen3_moe_c4_500m_aux_loss.py "${common_overrides[@]}"
fi
