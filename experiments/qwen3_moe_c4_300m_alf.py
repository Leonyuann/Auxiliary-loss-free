"""324M-parameter 16-expert Qwen3 MoE C4 ALF sign bias-update experiment."""

from alf.config import AlfConfig, DataConfig, EvalConfig, ExperimentConfig, ModelConfig, TrainingConfig, WandbConfig

config = ExperimentConfig(
    name="qwen3_moe_c4_300m_alf",
    model=ModelConfig(
        use_tiny_config=True,
        tokenizer_name_or_path="/vepfs-mlp2/ylq/tokenizers/owt_bpe_32k",
        vocab_size=32768,
        hidden_size=512,
        intermediate_size=1280,
        num_hidden_layers=9,
        num_attention_heads=8,
        num_key_value_heads=4,
        num_experts=16,
        num_experts_per_tok=2,
        torch_dtype="bfloat16",
    ),
    data=DataConfig(
        train_files=["/vepfs-mlp2/ylq/data/c4/c4_train_owt_bpe32k_tokens.i32"],
        validation_files=["/vepfs-mlp2/ylq/data/c4/c4_validation_owt_bpe32k_tokens.i32"],
        block_size=512,
        max_train_samples=None,
        max_validation_samples=32_768,
    ),
    eval=EvalConfig(eval_every=1000, eval_batch_size=32, max_eval_samples=2048),
    training=TrainingConfig(
        output_dir="outputs/qwen3_moe_c4_300m_alf",
        seed=42,
        max_steps=100_000,
        batch_size=32,
        gradient_accumulation_steps=2,
        learning_rate=3e-4,
        weight_decay=0.1,
        scheduler_type="cosine",
        warmup_steps=2000,
        max_grad_norm=1.0,
        optimizer_state_dtype="float32",
        log_every=10,
        save_every=2500,
        device="auto",
        num_workers=4,
        pin_memory=True,
        drop_last=True,
        gradient_checkpointing=False,
        ddp_backend="nccl",
        ddp_find_unused_parameters=False,
    ),
    alf=AlfConfig(
        enabled=True,
        bias_update_rate=5e-4,
        bias_update_policy="sign",
        bias_ema_beta=0.9,
        bias_update_schedule="linear",
        bias_update_schedule_steps=200_000,
        bias_update_end_rate=1e-4,
        bias_init=0.0,
        bias_clip=2.0,
        update_interval=2,
        warmup_steps=4000,
        disable_router_aux_loss=True,
    ),
    wandb=WandbConfig(
        enabled=True,
        entity="liangqingyuann-huazhong-university-of-science-and-technology",
        project="Load-balance",
        group="c4-300m",
        tags=["alf", "alf-sign", "qwen3-moe", "c4", "300m-family", "324m", "16experts", "bpe32k", "ddp"],
    ),
)
